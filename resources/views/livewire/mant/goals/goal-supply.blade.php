   ...
   <article class="shadow-lg border border-gray-100 rounded p-2">
        <div class="card">
            <div class="card-body">
                <h1 class="text-xl font-bold text-gray-500 capitalize">{{ __("add supplies") }}</h1>
                <hr class="mt-2 mb-3">

                <form method="post" class="text-xs" wire:submit.prevent="saveReplacement">
                    <select wire:model="supplyId" class="select w-full">
                       <option value="">{{ __("select supply") }}</option>
